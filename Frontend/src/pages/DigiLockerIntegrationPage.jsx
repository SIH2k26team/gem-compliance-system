  ...
  return (
    <AppLayout role="bidder" currentPath={currentPath} navigate={navigate}>
      <div className="space-y-5">
        ...
        <div className="bg-slate-900 text-white rounded-md p-5 shadow-2xs border border-slate-800 relative overflow-hidden">
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-5 relative z-10">
            ...
            <div className="flex flex-col sm:flex-row lg:flex-col gap-2 shrink-0">
              <button
                ...
                disabled={isSyncing}
                className="px-4 py-2 bg-emerald-800 hover:bg-emerald-700 text-white font-bold rounded text-xs flex items-center justify-center gap-2 shadow-2xs transition-all cursor-pointer disabled:opacity-50"
              >
                <svg className={`w-4 h-4 ${isSyncing ? 'animate-spin' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
                </svg>
                <span>{isSyncing ? 'Synchronizing Vault...' : 'Sync Vault with DigiLocker API'}</span>
              </button>

              <button
                onClick={() => setShowLinkModal(true)}
                className="px-4 py-2 bg-slate-800 hover:bg-slate-700 border border-slate-700 text-white font-bold rounded text-xs flex items-center justify-center gap-2 transition-all cursor-pointer"
              >
